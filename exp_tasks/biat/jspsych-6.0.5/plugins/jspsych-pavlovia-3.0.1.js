...
jsPsych.plugins['pavlovia'] = (function() {
	...
	plugin.info = {
		...
		parameters: {
			...
			participant_id: {
				...
				default: 'PARTICIPANT',
				description: 'to include participant specific name in datafile: "PARTICIPANT" (default) or any string you like'
			},
			errorCallback: {
	...
	const _finish = async function(trial, data) {
		try {
			...
			const key = _config.experiment.name + '_' + trial.participant_id + '_' + 'SESSION' + '_' + dateString + '.csv';
